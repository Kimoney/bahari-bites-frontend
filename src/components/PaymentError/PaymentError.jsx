        ...
        <div className="p-4 sm:p-6 lg:p-8 mt-12">
            <div className="max-w-md mx-auto lg:max-w-3xl">
                <div className="border border-orange-500 rounded-lg shadow-sm p-4 mt-10 bg-white">
                    ...
                    <div className="flex flex-col sm:flex-row justify-between mt-5 mb-5 p-4 border rounded-lg border-gray-300 bg-red-300">
                        <p className="text-gray-700 capitalize">The STK Push to your M-Pesa Number was unsuccessful. Please try again.</p>
                    </div>
                    <div className="flex justify-center mt-8">
                        <Link to="/payment-loading" className="w-full">
                            <button 
                                className={`border font-extrabold border-orange-500 text-orange-500 p-4 rounded-lg w-full ${isButtonDisabled ? 'bg-gray-300 cursor-not-allowed' : 'hover:bg-orange-500 hover:text-white'}`}
                                disabled={isButtonDisabled}
                            >
                                {isButtonDisabled ? `Retry in ${countdown} seconds` : `PAY NOW KES ${totalPrice.toFixed(2)}`}
                            </button>
                        </Link>
                    </div>
